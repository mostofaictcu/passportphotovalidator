#!/usr/bin/env python3
"""
Passport Photo Validator Core Module v2.0 for Bangladesh
...
class PassportPhotoValidatorV2:
    """Enhanced passport photo validator with sharpness, light balance, and face edge detection"""
    ...
    def __init__(self, image_path):
        ...
        # V2 Standards
        self.MIN_SHARPNESS = 150
        self.LIGHT_BALANCE_THRESHOLD = 80
        self.MIN_RIGHT_MARGIN = 0.15
        self.MIN_BOTTOM_MARGIN = 0.15
    ...
    def check_photo_sharpness(self):
        """V2: Check if photo has sufficient sharpness"""
        try:
            laplacian = cv2.Laplacian(self.gray, cv2.CV_64F)
            sharpness_score = laplacian.var()
            is_sharp = sharpness_score >= self.MIN_SHARPNESS
            ...
            self.validation_results['sharpness'] = {
                'valid': bool(is_sharp),
                'laplacian_variance': round(float(sharpness_score), 2),
                'min_required': self.MIN_SHARPNESS,
                'status': 'Sharp' if is_sharp else 'Blurry/Bad Quality',
                'issue_code': 'ISSUE_PHOTO_SHARPNESS_BAD' if not is_sharp else None
            }
            logger.info(f"Sharpness check: {sharpness_score:.2f} (threshold: {self.MIN_SHARPNESS})")
            return bool(is_sharp)
        except Exception as e:
    ...
    def check_face_light_balance(self, face_region=None):
        """V2: Check if face has balanced lighting"""
        try:
            ...
            self.validation_results['light_balance'] = {
                ...
                'threshold_variance': round(float(self.LIGHT_BALANCE_THRESHOLD ** 2), 2),
                'balanced_lr': bool(balanced_lr),
                'balanced_tb': bool(balanced_tb),
                'even_variance': bool(even_variance),
                'issue_code': 'ISSUE_FACE_LIGHT_NOT_BALANCE' if not is_balanced else None
            }
            logger.info(f"Light Balance - LR Diff: {lr_diff:.2f}, TB Diff: {tb_diff:.2f}, Variance: {overall_variance:.2f}")
    ...
    def check_face_brightness(self, face_region=None):
        """V2: Check if face brightness is adequate"""
        try:
    ...
    def check_right_side_visibility(self, faces):
        """V2: Check if face right side is fully visible"""
        if len(faces) == 0:
    ...
    def check_left_side_visibility(self, faces):
        """V2: Check if face left side is fully visible"""
        if len(faces) == 0:
            return False, {'valid': False, 'message': 'No face detected', 'issue_code': None}
    ...
    def check_bottom_face_visibility(self, faces):
        """V2: Check if face bottom is fully visible"""
        if len(faces) == 0:
            return False, {'valid': False, 'message': 'No face detected', 'issue_code': None}
